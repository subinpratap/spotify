...
class Header extends Component {
    ...
    openSongsDetails = (songId) => {
        this.props.playSong(`spotify:track:${songId}`);
        this.showSearchResults = false;
        this.setState({
            'searchTerm': '',
            'showSearchResults': false
        ...
        return (
            <section className="headerComponent">
                <nav className="mb-1 navbar navbar-expand-lg navbar-dark secondary-color lighten-1">
                    <Link className="navbar-brand" to="/home">Spotify</Link>
                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent-555">
                        <ul className="navbar-nav mr-auto">
                            <li className={'nav-item' + (this.props && this.props.match && this.props.match.path.includes('home') ? 'active' : '')}>
                                <Link to="/home" className="nav-link">Home</Link>
                            </li>
                            <li className={'nav-item' + (this.props && this.props.match && this.props.match.path.includes('myMusic') ? 'active' : '')}>
                                <Link to="/myMusic" className="nav-link">My Music</Link>
                            </li>
                        </ul>
...
export default connect(
    mapStateToProps,
    { logout, searchArtist, searchSong, playSong }
)(Header);